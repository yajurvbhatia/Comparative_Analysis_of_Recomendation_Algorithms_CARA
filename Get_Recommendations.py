...
def get_random_rec(k, m):
    st = time.time()
    item_ids = [i for i in range(m)]
    recommendations = list()
    for j in range(k):
        recommendations.append(random.randint(1,m+1))
    en = time.time()
    return recommendations, en-st

def get_popular_rec(ratings, k):
    st = time.time()
...
def get_content_filtered_rec(user_id, ratings, m_profiles, k, nb_genres):
    ...
    for r in range(k):
        ...
        recommendations.append(top_scorer)
        estimated_ratings[top_scorer-1] = -999
    en = time.time()
    return recommendations, en-st

def get_user_based_collab_filtered_rec(user_id, ratings, u, k, m):
    st = time.time()
    ...
    u1_ratings = ratings[user_id-1]
    cosine_distances = [0 for x in range(ratings.shape[0])]
...
def get_item_based_collab_filtered_rec(user_id, ratings, n, k, m):
    st = time.time()
    ...
    for i in rated_items:
        i1_vector = ratings[i-1]
        cosine_distances = [0 for x in range(ratings.shape[0])]
        for i2_id in range(1,ratings.shape[0]):
            if not (i2_id == i):
                i2_vector = ratings[i2_id-1]
                cosine_distances[i2_id-1] = spatial.distance.cosine(i1_vector, i2_vector)

        for j in range(n):
            item_id = np.argmax(cosine_distances) + 1
            similarity_score = cosine_distances[item_id-1]
            cosine_distances.remove(similarity_score)
...
def get_category_based_rec(user_id, ratings, m_profiles, k, nb_genres):
    st = time.time()
    u1_ratings = ratings[user_id-1]
    rated_items = list()
    ...
    en = time.time()
    return recommended_items[:k], en-st
